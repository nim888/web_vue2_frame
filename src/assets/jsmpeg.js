...
JSMpeg.Source.WebSocket = function() {
    ...
    WSSource.prototype.onOpen = function(e) {
        this.call('open');
        this.progress = 1;
        this.established = true;
    };
    ...
    WSSource.prototype.onMessage = function(ev) {
        this.call('message', ev);
        if (this.destination) {
            var arr = new Uint8Array(ev.data);
            var id = arr[arr.byteLength - 1];
            var data = arr.slice(0, -1);
            this.destination.write(data.buffer);
        }
    };

    WSSource.prototype.call = function(name, e) {
